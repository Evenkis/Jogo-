...
features:   -jogador do pode ter 3 tiros na tela ao mesmo tempo
            -inimigos "mostram" que v�o atirar antes de atirar"
            -inimigos andam, se estiveram a uma certa distancia, na dire��o do jogador
            ...
            -os inimigos detectam paredes e n�o batem nelas
...
int main()
{
    ...
    tiros_inimigos_t tiros[INIMIGO_MAX] = {0}; //vetor posi��o dos tiros, 1 tiro por inimigo
...
//        printf("nave_linha = %d , nave_coluna = %d , limite = %d ", pos.linhaY, pos.colunaX, limite);
        ...
        if((pos.colunaX + COLUNAS_TELA )>= COLUNAS )
        {
            ...
            mapa[pos.linhaY][310] = ' ';
            mapa[(pos.linhaY)- 1][310] = ' '; ///s� uns numeros magicos para funcionar o mapa 100%
        ...
        {
            limite = pos.colunaX - x_nave_zero; ///s� uns numeros magicos para funcionar o mapa 100%

        }
        x_nave = pos.colunaX;
        y_nave = pos.linhaY;

        if((i%3 == 0) && acertou ==0) /// a cada 3 interacoes, isso sera verdadeiro e os inimigos vao se mover
        ...
        if(morreu == 0)
            detecta_colisao_tiros_inimigos(mapa,pos,vel,&morreu);
        ...
        faz_tiros_andarem(mapa,vet_tiros,quantos_tiros); //do jogador
        ...
        if(i%3 == 0)
        {
            inimigos_atiram(mapa,tiros,inimigos,n_inimigos);
