...
void detecta_colisao_tiros_inimigos(char mapa[][COLUNAS],posica_t pos,int vel,int *morreu)
{
    int i;

    if(mapa[pos.linhaY -1][pos.colunaX +vel] == '~')
    {
        *morreu = -1;
    }
    ...
    for(i = 0; i < 4; i++)                  ///mesma coisa q colisao com mapa
    {
        if ((mapa[pos.linhaY][pos.colunaX +vel +i] == '~'))
        {
            *morreu = -1;
        }
    }
...
int ve_se_tiro_bateu_inimigo(char mapa[][COLUNAS],posicao_t inimigos[],int n_inimigos,int *acertou)
{
    int i,k,l,contador_de_acertos=0;
            ...
            for(l=0; l<2; l++)
                if( mapa[inimigos[i].linhaY - l][inimigos[i].colunaX + k] == '-')
                {
                    *acertou = 1;
                    ...
                    contador_de_acertos++;                                            // apaga ele
                }                                                       // 3) pode ser uma maneira de add os pontos

        return contador_de_acertos;
}
...
void coloca_a_nave_no_meio_e_arruma_mapa(char mapa[][COLUNAS],posica_t pos,int *linha_da_nave)
{
    ...
    *linha_da_nave = 18; ///bota a nvae no meio do mapa novamente
